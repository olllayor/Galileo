...
export const findParentNode = (doc: Document, nodeId: string): Node | null => {
...
export const updateNode = (doc: Document, nodeId: string, updates: Partial<Node>): Document => {
	const node = doc.nodes[nodeId];
	if (!node) {
		throw new Error(`Node ${nodeId} not found`);
	}
...
export const moveNode = (doc: Document, nodeId: string, newParentId: string, newIndex?: number): Document => {
	const node = doc.nodes[nodeId];
